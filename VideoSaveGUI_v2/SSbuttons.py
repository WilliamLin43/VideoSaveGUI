...
class SSbuttons:
    def __init__(self, world):
        ...
        self.videostream=None#VideoStream().start()
    ...
    def update(self):
        ...
        if self.recording:
            cv2.imshow("test",self.videostream.read())
        else:
            ...
            if self.world.mouse_left_down:
                if self.recording:
                    ...
                    if not self.filepath=="" and not len(self.filename)==0:
                        ...
                        self.videostream=VideoStream(self.filepath, fname, self.resolutions[self.resolutionselect]).start()
